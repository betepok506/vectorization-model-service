...
create-celery-db:
	if ! docker compose -f docker-compose-dev.yml exec database psql -U ${POSTGRESQL_USERNAME} -h localhost -lqt | cut -d \| -f 1 | grep -qw ${DATABASE_CELERY_NAME}; then \
		docker compose -f docker-compose-dev.yml exec database createdb -U ${POSTGRESQL_USERNAME} -W ${POSTGRESQL_PASSWORD} -h localhost -O ${POSTGRESQL_USERNAME} ${DATABASE_CELERY_NAME}; \
	fi
